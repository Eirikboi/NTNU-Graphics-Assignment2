...
static int IMGUI_CDECL ShrinkWidthItemComparer(const void* lhs, const void* rhs)
{
    ...
    if (int d = (int)(b->textureWidth - a->textureWidth))
        return d;
...
void ImGui::ShrinkWidths(ImGuiShrinkWidthItem* items, int count, float width_excess)
{
    if (count == 1)
    {
        items[0].textureWidth = ImMax(items[0].textureWidth - width_excess, 1.0f);
        return;
    }
    ...
    {
        while (count_same_width < count && items[0].textureWidth <= items[count_same_width].textureWidth)
            count_same_width++;
        float max_width_to_remove_per_item = (count_same_width < count) ? (items[0].textureWidth - items[count_same_width].textureWidth) : (items[0].textureWidth - 1.0f);
        float width_to_remove_per_item = ImMin(width_excess / count_same_width, max_width_to_remove_per_item);
        for (int item_n = 0; item_n < count_same_width; item_n++)
            items[item_n].textureWidth -= width_to_remove_per_item;
    ...
    {
        float width_rounded = ImFloor(items[n].textureWidth);
        width_excess += items[n].textureWidth - width_rounded;
        items[n].textureWidth = width_rounded;
    }
    if (width_excess > 0.0f)
        for (int n = 0; n < count; n++)
            if (items[n].Index < (int)(width_excess + 0.01f))
                items[n].textureWidth += 1.0f;
}
...
{
    Spacing = textureWidth = NextWidth = 0.0f;
...
void ImGuiMenuColumns::Update(int count, float spacing, bool clear)
{
    ...
    for (int i = 0; i < IM_ARRAYSIZE(Pos); i++)
    {
        if (i > 0 && NextWidths[i] > 0.0f)
            textureWidth += Spacing;
        Pos[i] = IM_FLOOR(textureWidth);
        textureWidth += NextWidths[i];
        NextWidths[i] = 0.0f;
    }
}
...
float ImGuiMenuColumns::DeclColumns(float w0, float w1, float w2) // not using va_arg because they promote float to double
{
    ...
    return ImMax(textureWidth, NextWidth);
}

float ImGuiMenuColumns::CalcExtraSpace(float avail_w) const
{
    return ImMax(0.0f, avail_w - textureWidth);
}
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        g.ShrinkWidthBuffer[tab_n].textureWidth = tab->ContentWidth;
    }
    ...
    if (width_excess > 0.0f && (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown))
    {
        ...
        for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
            tab_bar->Tabs[g.ShrinkWidthBuffer[tab_n].Index].textureWidth = IM_FLOOR(g.ShrinkWidthBuffer[tab_n].textureWidth);
    }
    else
    {
        ...
        {
            ImGuiTabItem* tab = &tab_bar->Tabs[tab_n];
            tab->textureWidth = ImMin(tab->ContentWidth, tab_max_width);
            IM_ASSERT(tab->textureWidth > 0.0f);
        }
    }
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        offset_x += tab->textureWidth + g.Style.ItemInnerSpacing.x;
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    ...
    float tab_x2 = tab->Offset + tab->textureWidth + (order + 1 < tab_bar->Tabs.Size ? margin : 1.0f);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (!tab_appearing && !(tab_bar->Flags & ImGuiTabBarFlags_Reorderable))
    {
        ...
        tab_bar->OffsetNextTab += tab->textureWidth + g.Style.ItemInnerSpacing.x;
    }
