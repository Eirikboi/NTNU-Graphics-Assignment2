...
static void ShowDemoWindowWidgets()
{
    ...
    if (ImGui::TreeNode("Drag and Drop"))
    {
        ...
        if (ImGui::TreeNode("Drag and drop to copy/swap items"))
        {
            ...
            static const char* blockVariableNames[9] = { "Bobby", "Beatrice", "Betty", "Brianna", "Barry", "Bernard", "Bibi", "Blaine", "Bryn" };
            for (int n = 0; n < IM_ARRAYSIZE(blockVariableNames); n++)
            {
                ImGui::PushID(n);
                if ((n % 3) != 0)
                    ImGui::SameLine();
                ImGui::Button(blockVariableNames[n], ImVec2(60,60));

                // Our buttons are both drag sources and drag targets here!
                if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None))
                {
                    ImGui::SetDragDropPayload("DND_DEMO_CELL", &n, sizeof(int));    // Set payload to carry the index of our item (could be anything)
                    if (mode == Mode_Copy) { ImGui::Text("Copy %s", blockVariableNames[n]); }    // Display preview (could be anything, e.g. when dragging an image we could decide to display the filename and a small preview of the image, etc.)
                    if (mode == Mode_Move) { ImGui::Text("Move %s", blockVariableNames[n]); }
                    if (mode == Mode_Swap) { ImGui::Text("Swap %s", blockVariableNames[n]); }
                ...
                if (ImGui::BeginDragDropTarget())
                {
                    if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("DND_DEMO_CELL"))
                    {
                        ...
                        if (mode == Mode_Copy)
                        {
                            blockVariableNames[n] = blockVariableNames[payload_n];
                        }
                        if (mode == Mode_Move)
                        {
                            blockVariableNames[n] = blockVariableNames[payload_n];
                            blockVariableNames[payload_n] = "";
                        }
                        if (mode == Mode_Swap)
                        {
                            const char* tmp = blockVariableNames[n];
                            blockVariableNames[n] = blockVariableNames[payload_n];
                            blockVariableNames[payload_n] = tmp;
                        }
                    }
                    ImGui::EndDragDropTarget();
                }
...
static void ShowDemoWindowLayout()
{
    ...
    if (ImGui::TreeNode("Tabs"))
    {
        ...
        if (ImGui::TreeNode("Advanced & Close Button"))
        {
            ...
            // Tab Bar
            const char* blockVariableNames[4] = { "Artichoke", "Beetroot", "Celery", "Daikon" };
            static bool opened[4] = { true, true, true, true }; // Persistent user state
            for (int n = 0; n < IM_ARRAYSIZE(opened); n++)
            {
                if (n > 0) { ImGui::SameLine(); }
                ImGui::Checkbox(blockVariableNames[n], &opened[n]);
            }
            ...
            {
                for (int n = 0; n < IM_ARRAYSIZE(opened); n++)
                    if (opened[n] && ImGui::BeginTabItem(blockVariableNames[n], &opened[n], ImGuiTabItemFlags_None))
                    {
                        ImGui::Text("This is the %s tab!", blockVariableNames[n]);
                        if (n & 1)
    ...
    if (ImGui::TreeNode("Scrolling"))
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            if (i > 0) ImGui::SameLine();
            ImGui::BeginGroup();
            const char* blockVariableNames[] = { "Top", "25%", "Center", "75%", "Bottom" };
            ImGui::TextUnformatted(blockVariableNames[i]);
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            const char* blockVariableNames[] = { "Left", "25%", "Center", "75%", "Right" };
            ImGui::Text("%s\n%.0f/%.0f", blockVariableNames[i], scroll_x, scroll_max_x);
            ImGui::Spacing();
        }
        ImGui::PopID();
...
static void ShowDemoWindowPopups()
{
    ...
    if (ImGui::TreeNode("Popups"))
    {
        ...
        const char* blockVariableNames[] = { "Bream", "Haddock", "Mackerel", "Pollock", "Tilefish" };
        ...
        ImGui::SameLine();
        ImGui::TextUnformatted(selected_fish == -1 ? "<None>" : blockVariableNames[selected_fish]);
        if (ImGui::BeginPopup("my_select_popup"))
        {
            ImGui::Text("Aquarium");
            ImGui::Separator();
            for (int i = 0; i < IM_ARRAYSIZE(blockVariableNames); i++)
                if (ImGui::Selectable(blockVariableNames[i]))
                    selected_fish = i;
            ImGui::EndPopup();
        }
        ...
        {
            for (int i = 0; i < IM_ARRAYSIZE(blockVariableNames); i++)
                ImGui::MenuItem(blockVariableNames[i], "", &toggles[i]);
...
static void ShowDemoWindowColumns()
{
    ...
    if (ImGui::TreeNode("Basic"))
    {
        ...
        ImGui::Separator();
        const char* blockVariableNames[3] = { "One", "Two", "Three" };
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            ImGui::NextColumn();
            ImGui::Text(blockVariableNames[i]); ImGui::NextColumn();
