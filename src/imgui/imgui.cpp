...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    const bool first_begin_of_the_frame = (window->LastFrameActive != currentFrame);
    window->IsFallbackWindow = (g.CurrentWindowStack.Size == 0 && g.WithinFrameScopeWithImplicitWindow);

    // Update the Appearing flag
    bool window_just_activated_by_user = (window->LastFrameActive < currentFrame - 1);   // Not using !WasActive because the implicit "Debug" window would always toggle off->on
...
void ImGui::SetNextItemWidth(float item_width)
{
    ...
    g.NextItemData.textureWidth = item_width;
}
