...
namespace
{
    ...
    struct GlyphInfo
    {
        int         textureWidth;              // Glyph's width in pixels.
        int         textureHeight;             // Glyph's height in pixels.
    ...
    const FT_Bitmap* FreeTypeFont::RenderGlyphAndGetInfo(GlyphInfo* out_glyph_info)
    {
        ...
        out_glyph_info->textureWidth = (int)ft_bitmap->width;
        out_glyph_info->textureHeight = (int)ft_bitmap->rows;
...
bool ImFontAtlasBuildWithFreeType(FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags)
{
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsList.Size; glyph_i++)
        {
            ...
            const int bitmap_size_in_bytes = src_glyph.Info.textureWidth * src_glyph.Info.textureHeight;
            ...
            src_tmp.Font.BlitGlyph(ft_bitmap, src_glyph.BitmapData, src_glyph.Info.textureWidth * 1, multiply_enabled ? multiply_table : NULL);

            src_tmp.Rects[glyph_i].w = (stbrp_coord)(src_glyph.Info.textureWidth + padding);
            src_tmp.Rects[glyph_i].h = (stbrp_coord)(src_glyph.Info.textureHeight + padding);
    ...
    for (int src_i = 0; src_i < src_tmp_array.Size; src_i++)
    {
        ...
        for (int glyph_i = 0; glyph_i < src_tmp.GlyphsCount; glyph_i++)
        {
            ...
            IM_ASSERT(info.textureWidth + padding <= pack_rect.w);
            IM_ASSERT(info.textureHeight + padding <= pack_rect.h);
            const int tx = pack_rect.x + padding;
            const int ty = pack_rect.y + padding;

            // Blit from temporary buffer to final texture
            size_t blit_src_stride = (size_t)src_glyph.Info.textureWidth;
            size_t blit_dst_stride = (size_t)atlas->TexWidth;
            unsigned char* blit_src = src_glyph.BitmapData;
            unsigned char* blit_dst = atlas->TexPixelsAlpha8 + (ty * blit_dst_stride) + tx;
            for (int y = info.textureHeight; y > 0; y--, blit_dst += blit_dst_stride, blit_src += blit_src_stride)
            ...
            float x1 = x0 + info.textureWidth;
            float y1 = y0 + info.textureHeight;
            float u0 = (tx) / (float)atlas->TexWidth;
            float v0 = (ty) / (float)atlas->TexHeight;
            float u1 = (tx + info.textureWidth) / (float)atlas->TexWidth;
            float v1 = (ty + info.textureHeight) / (float)atlas->TexHeight;
